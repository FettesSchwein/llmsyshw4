...
void launch_attn_softmax_bw(float *out_grad, const float *soft_outp, int rows,
  ...
  int float_size = sizeof(float);
  int data_size = rows * softmax_len * float_size;

  float *d_out_grad, *d_soft_outp;
  cudaMalloc((void **)&d_out_grad, data_size);
  cudaMalloc((void **)&d_soft_outp, data_size);

  cudaMemcpy(d_out_grad, out_grad, data_size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_soft_outp, soft_outp, data_size, cudaMemcpyHostToDevice);
  ...
  if (softmax_len <= 32) {
    ...
    printf("Softmax length %d not supported\n", softmax_len);
  }
