...
class PowerScalar(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        grad_a = grad_output * scalar * (a ** (scalar - 1.0))
        return (grad_a, 0.0)
...
class Attn_Softmax(Function):
    ...
    def forward(ctx: Context, inp: Tensor, mask: Tensor) -> Tensor:
      #   BEGIN ASSIGN4_1_1
      soft_out = inp.f.attn_softmax_fw(inp, mask)
      ctx.save_for_backward(soft_out)
      return soft_out
      #   END ASSIGN4_1_1
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tensor:
      ...
      grad_inp = out_grad.f.attn_softmax_bw(out_grad, soft_out)
      return grad_inp, 0.0
      #   END ASSIGN4_1_2
...
class LayerNorm(Function):
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tensor:
      ...
      gamma_grad, beta_grad, inp_grad = out_grad.f.layernorm_bw(out_grad, inp, gamma, beta, var, mean)
